...
app.post('/api/ai', async (req, res) => {
  ...
  const formattingNote = "\n\n(IMPORTANT: Answer in clear, friendly, natural language. Do not output raw LaTeX like $\\text{...}$, $$, or complex code blocks unless specifically requested. Use simple plain text and clean Unicode like H₂O, CO₂, O₂, glucose).";
  const userPrompt = prompt.includes('IMPORTANT:') ? prompt : `${prompt}${formattingNote}`;
  ...
  for (const model of candidateModels) {
    try {
      ...
      console.warn(`[Gemini Proxy] Model ${model} failed (${response.status}): ${lastError}. Attempting next candidate...`);
    } catch (err) {
      lastError = err.message;
      console.warn(`[Gemini Proxy] Network error with ${model}:`, err.message);
    }
  }

  return res.status(500).json({ error: lastError || 'All Gemini candidate models failed.' });
});
  ...
  ws.on('message', (messageAsString) => {
    try {
      ...
      switch (data.type) {
        ...
        case 'sticky_batch_sync':
          if (currentRoom && roomsMap.has(currentRoom)) {
            const room = roomsMap.get(currentRoom);
            room.stickyNotes = data.stickyNotes || [];
            broadcast(currentRoom, { ...data, senderId: clientId }, ws);
          }
          break;
